...
await renderMonorepoConventionalCommits(
  parsed,
  (project, versions) => {
    if (project.isRoot || project.package.name.endsWith('docs')) return
      ...
      const sections = groups.map((group) => {
        ...
        const commits = changes
          ?.map((commit) => {
            ...
            return `- ${commit.variables.description} ([${shortHash}](https://github.com/jrson83/shrtcss/commit/${shortHash}))`
          })
          .join('\n')
        return `### ${group}\n\n${commits}`
      })
